...
def recommend_music_by_weather(city):
    ...
    print(f"해석된 날씨: {weather_main}")

    if not weather_main:
        print("날씨 데이터를 가져오지 못했습니다.")
        return {"weather": "정보없음", "genre": "unknown", "tracks": []}
    ...
    if not tracks:
        print("추천 트랙이 없습니다.")
        return {"weather": weather_main, "genre": selected_genre, "tracks": []}
...
def recommend_music_by_user(user):
    ...
    "recent": [(track['artist']['#text'], track['name']) for track in recent_tracks[:2]]
}
